...
userinit(void)
{
  ...
  p->sz = PGSIZE;
  pt2kptcopy(p->pagetable, p->kpagetable, 0, p->sz);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    pt2kptcopy(p->pagetable, p->kpagetable, sz - n, n);
  } else if(n < 0){
...
fork(void)
{
  ...
  np->sz = p->sz;
  pt2kptcopy(np->pagetable, np->kpagetable, 0, np->sz);
